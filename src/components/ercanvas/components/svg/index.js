...
export default React.memo(({isArrow = true, type, reverse = false, prefix, className = '', ...restProps}) => {
    const currentPrefix = getPrefix(prefix);
    return <div {...restProps} className={`${currentPrefix}-er-svg-icon ${className}`}>
      <svg style={{width: '32px', height: '32px', transform: `rotate(${reverse ? '180' : '0'}deg)`}}>
        {
